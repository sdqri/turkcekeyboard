////////////////////////////////////////////////////////////////////////////////////
///////////////////////////added from main page for writing comment/////////////////

...
jQuery.fn.extend({
	...
	clearAtCursor: function(cps){
    	...
    	if(t.length==cps){
        	...
        	this.val(t.substring(0,pos-1)+t.substring(pos,t.length))
...
var keylabel_sh={
    ...
    "k2-12":"",
    "k1-0":"","k1-1":"ض","k1-2":"ك","k1-3":"ً","k1-4":"ُ","k1-5":"ِ",
...
var keymap_sh={
    ...
    "k2-6":"ح","k2-7":"ظ","k2-8":"»","k2-9":"«","k2-10":"اؽ","k2-11":":",
    "k2-12":"\n",
    "k1-0":"shift","k1-1":"ض","k1-2":"ك","k1-3":"ً","k1-4":"ُ","k1-5":"ِ",
...
$(document).ready(function() {
	...
	var posting=$.post('captcha_set.php');
	posting.done(function(data){
		var pt="امنیتی سوْرغۇ : ";
		$("#lbl_captcha").text(pt+data);
	});
	...
	*/
	$('#captcha').bind('input', function(e){
		//diabling submit button
		$("#btn_submit").addClass('disabled');
		$("#captcha-span").removeClass("glyphicon-ok");
		$("#captcha-span").removeClass("glyphicon-remove");
		$("#captcha-span").removeClass('glyphicon-question-sign').addClass('glyphicon-transfer');
		$("#captcha-span").css('color', 'blue');
		var ct=$("#captcha").val();
		if(ct==""){
			$("#captcha-span").css('color','#555');
			$("#captcha-span").removeClass('glyphicon-transfer').addClass('glyphicon-question-sign');
		}
		else{
			var posting=$.post('captcha_check.php',{captcha:ct});
			posting.done(function(data){
				if(data=="1"){
					$("#captcha-span").css('color', 'green');
					$("#captcha-span").removeClass('glyphicon-transfer').addClass('glyphicon-ok');
					if( !($("#btn_submit").hasClass("btn-danger") || $("#btn_submit").hasClass("btn-success")) ){
						$("#btn_submit").removeClass('disabled');
					}
				}
				else{
					$("#captcha-span").css('color','red');
					$("#captcha-span").removeClass('glyphicon-transfer').addClass('glyphicon-remove');
				}
			});
		}
	});
	...
	$("#btn_submit").click(function(event) {
		if(!$("#btn_submit").hasClass('disabled')){
			...
			var cm=$("#comment").val();
			var c=$("#captcha").val();
			var posting=$.post('ilgi-action.php',{name:n,email:e,blog:b,comment:cm,captcha:c});
			posting.done(function(data){
				if(data=="1"){
					$("#btn_submit").addClass("btn-success");
				$("#btn_submit").addClass('disabled');
				$("#btn_submit").val("پئیغامؽنؽز گؤندریلدی!");
				}
			else{
					$("#btn_submit").addClass("btn-danger");
					$("#btn_submit").addClass('disabled');
					var t1="تاسۆفله‌ بیر مۆشکۆل قارشؽمؽزا چؽخدؽ!";
					$("#btn_submit").text(t1);
				}
			});
		}
...
////////////////////////////////////////////////////////////////////////////////////
   ...
   $("#comment").keypress(function(e){
        if(!(e.ctrlKey)){
            if(keyidbycode_kp.hasOwnProperty(e.which) || keyidbycode_kp_sh.hasOwnProperty(e.which)){
                ...
                $("#comment").val(text.substring(0,text.length-1));
            }
        }
        else{
   ...
   $("#comment").keydown(function(e){
       var cps=$("#comment").getCursorPosition();
       var keycode=e.which;
       if(keyidbycode.hasOwnProperty(keycode) && !(e.ctrlKey)){
            ...
            //backspace key
                ...
                $("#comment").clearAtCursor($("#comment").getCursorPosition());
            }
            //for tab key
            else if(keycode===0){
            }
            ...
            //letter keys
                ...
                if(shift===true){
                    $("#comment").insertAtCursor(keymap_sh[keyidbycode[keycode]]);
                    shift = !shift;
                    setShiftedView();
                }
                ...
                //letter key without any other key like shift or caps lock
                    ...
                    $("#comment").insertAtCursor(keymap[keyidbycode[keycode]]);
                }
            }
       }
   });
